...
| Command | Behavior |
|---------|----------|
| `python3 scripts/generate_metrics.py > docs/$(date +%Y-%m-%d)_codebase_metrics.md` | Fast: no coverage data (Cover column shows `-`) |
| `python3 scripts/generate_metrics.py --with-coverage > docs/$(date +%Y-%m-%d)_codebase_metrics.md` | Slow (~3 min): uses cached `tarpaulin-report.json` if available, otherwise runs `cargo tarpaulin --all-features -o Json`. Includes real line coverage percentages. |
| `python3 scripts/generate_metrics.py --force-coverage > docs/$(date +%Y-%m-%d)_codebase_metrics.md` | Forces a fresh tarpaulin run (deletes cache first). Implies `--with-coverage`. |
