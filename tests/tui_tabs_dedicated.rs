...
fn test_tab_content_peers() {
    let content = TabContent::Peers(vec![
        ("peer-id".to_string(), "peer-name".to_string()),
    ]);
    match content {
...
fn test_tab_content_log() {
    let content = TabContent::Log(vec![
...
fn test_tab_content_dm() {
    let content = TabContent::Dm(vec![
        ("sender".to_string(), "message".to_string(), false),
    ]);
    match content {
