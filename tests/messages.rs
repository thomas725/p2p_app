...
fn test_save_direct_message() {
    with_test_db(|| {
        let msg =
            p2p_app::save_message("dm content", None, "chat", true, Some("peer-xyz")).unwrap();
...
fn test_save_receipt_different_kinds() {
    with_test_db(|| {
        p2p_app::save_receipt("msg-k", "peer-k", 0, 1.0).unwrap();
        p2p_app::save_receipt("msg-k", "peer-k", 1, 2.0).unwrap();
