    ...
    fn peer_sort_and_upsert_keep_selection() {
        let mut peers = VecDeque::from([
            ("a".to_string(), "t1".to_string(), "2024-01-01T00:00:01".to_string()),
            ("b".to_string(), "t1".to_string(), "2024-01-01T00:00:02".to_string()),
        ]);
    ...
    fn scroll_helpers() {
        ...
        assert_eq!(handle_scroll_key_for_section("Down", 0, false, 9), (1, false));
        assert_eq!(handle_scroll_key_for_section("PageUp", 9, false, 9), (1, false));
        assert_eq!(handle_scroll_key_for_section("PageDown", 0, false, 9), (8, false));
        assert_eq!(handle_scroll_key_for_section("Home", 4, true, 9), (0, false));
        assert_eq!(handle_scroll_key_for_section("End", 4, false, 9), (9, true));
        assert_eq!(handle_scroll_key_for_section("Unknown", 3, false, 9), (3, false));
