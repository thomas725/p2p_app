                                ...
                                && let Ok(event) = read()
                            {
                                match event {
                                    Event::Mouse(mouse) => {
                                        ...
                                        log_debug(&logs, format!("Mouse: row={}, col={}", row, col));

                                        if row == 0 && mouse_capture {
                                            ...
                                            if cnt > 0 {
                                                ...
                                                log_debug(&logs, format!("Tab: idx={}", idx));
                                        ...
                                        } else if matches!(dynamic_tabs.tab_index_to_content(active_tab), TabContent::Peers) && row > 1 && mouse_capture {
                                    ...
                                    Event::Key(key) => {
                                        ...
                                        } else if key.code == KeyCode::Esc || (key.modifiers.contains(KeyModifiers::CONTROL) && key.code == KeyCode::Char('q')) {
                                            execute!(std::io::stdout(), crossterm::event::DisableMouseCapture).ok();
                                        ...
                                        let input: Input = key.clone().into();
                                        ...
                                        if key.code == KeyCode::Enter && key.modifiers.contains(KeyModifiers::ALT) {
                                            if tab_content.is_input_enabled() {
                                                match &tab_content {
                                                    ...
                                                    TabContent::Direct(pid) => {
                                                        let dm = dm_inputs.entry(pid.clone()).or_insert_with(|| { let mut t = TextArea::default(); t.set_line_number_style(Style::default()); t.set_cursor_line_style(Style::default()); t });
                                                        dm.input_without_shortcuts(input);
                                                    }
                                                    _ => {}
                                                }
                                            }
                                            ...
                                            match tab_content {
                                                TabContent::Chat => {
                                                    ...
                                                    if !text.trim().is_empty() {
                                                        let ts = format_system_time(SystemTime::now());
                                                        let msg = format!("{} [You] {}", ts, text);
                                                        messages.push_back(msg);
                                                        let topic = gossipsub::IdentTopic::new("test-net");
                                                        let sent_at = SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_secs_f64();
                                                        let broadcast = p2p_app::BroadcastMessage { content: text.clone(), sent_at: Some(sent_at) };
                                                        let payload = serde_json::to_string(&broadcast).unwrap_or(text.clone());
                                                        swarm.behaviour_mut().gossipsub.publish(topic, payload.as_bytes()).ok();
                                                        save_message(&text, None, &topic_str, false, None).ok();
                                                    }
                                                ...
                                                TabContent::Peers if !peers.is_empty() => {
                                                    ...
                                                    if let Some((pid, _, _)) = peers.get(idx).cloned() {
                                                        active_tab = dynamic_tabs.add_dm_tab(pid.clone());
                                                        let msgs = dm_messages.entry(pid.clone()).or_default();
                                                        msgs.clear();
                                                    }
                                                }
                                                TabContent::Direct(pid) => {
                                                    ...
                                                    if !text.trim().is_empty() {
                                                        let ts = format_system_time(SystemTime::now());
                                                        let msgs = dm_messages.entry(pid.clone()).or_default();
                                                        msgs.push_back(format!("{} [You] {}", ts, text));
                                                        if let Ok(peer_id) = pid.parse::<libp2p::PeerId>() {
                                                            let dm = DirectMessage { content: text.clone(), timestamp: chrono::Utc::now().timestamp(), sent_at: Some(SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_secs_f64()) };
                                                            swarm.behaviour_mut().request_response.send_request(&peer_id, dm);
                                                            save_message(&text, None, &topic_str, true, Some(&peer_id.to_string())).ok();
                                                        }
                                                    }
                                                    *dm = TextArea::default();
                                                    dm.set_line_number_style(Style::default());
                                                    dm.set_cursor_line_style(Style::default());
                                                }
                                                _ => {}
                                            }
                                        } else if key.modifiers.contains(KeyModifiers::CONTROL) && key.code == KeyCode::Char('w') {
                                            if let TabContent::Direct(t) = tab_content {
                                                dynamic_tabs.remove_dm_tab(&t);
                                                dm_messages.remove(&t);
                                                dm_inputs.remove(&t);
                                                unread_dms.remove(&t);
                                            ...
                                            if unread_broadcasts > 0 {
                                                active_tab = 0;
                                                unread_broadcasts = 0;
                                            } else if let Some((t, _)) = unread_dms.iter().next() {
                                                let t = t.clone();
                                                active_tab = dynamic_tabs.add_dm_tab(t.clone());
                                                unread_dms.remove(&t);
                                            }
                                            ...
                                            match dynamic_tabs.tab_index_to_content(active_tab) {
                                                TabContent::Chat => { unread_broadcasts = 0; }
                                                TabContent::Direct(t) => { unread_dms.remove(&t); }
                                                _ => {}
                                            }
                                        } else if matches!(tab_content, TabContent::Chat) {
                                            match key.code {
                                                KeyCode::Up => { chat_auto_scroll = false; chat_scroll_offset = chat_scroll_offset.saturating_sub(1); }
                                                KeyCode::Down => { chat_scroll_offset = (chat_scroll_offset + 1).min(messages.len().saturating_sub(1)); }
                                                KeyCode::PageUp => { chat_auto_scroll = false; chat_scroll_offset = chat_scroll_offset.saturating_sub(20); }
                                                KeyCode::PageDown => { chat_scroll_offset = (chat_scroll_offset + 20).min(messages.len().saturating_sub(1)); }
                                                KeyCode::End => { chat_scroll_offset = usize::MAX; chat_auto_scroll = true; }
                                                KeyCode::Char(_) | KeyCode::Backspace | KeyCode::Delete | KeyCode::Left | KeyCode::Right | KeyCode::Home | KeyCode::Enter => { chat_input.input(input); }
                                                _ => {}
                                            }
                                        } else if let Some(pid) = tab_content.peer_id() {
                                            let dm = dm_inputs.entry(pid.to_string()).or_insert_with(|| { let mut t = TextArea::default(); t.set_line_number_style(Style::default()); t.set_cursor_line_style(Style::default()); t });
                                            match key.code {
                                                KeyCode::Char(_) | KeyCode::Backspace | KeyCode::Delete | KeyCode::Left | KeyCode::Right | KeyCode::Home | KeyCode::End | KeyCode::Enter => { dm.input(input); }
                                                _ => {}
                                            }
                                        } else if matches!(tab_content, TabContent::Peers) {
                                            match key.code {
                                                KeyCode::Up => { if !peers.is_empty() { peer_selection = peer_selection.saturating_sub(1); } }
                                                KeyCode::Down => { if !peers.is_empty() { peer_selection = (peer_selection + 1).min(peers.len() - 1); } }
                                                _ => {}
                                            }
                                        } else if matches!(tab_content, TabContent::Log) {
                                            match key.code {
                                                KeyCode::Up => { debug_auto_scroll = false; debug_scroll_offset = debug_scroll_offset.saturating_sub(1); }
                                                KeyCode::Down => { if let Ok(l) = logs.lock() { debug_scroll_offset = (debug_scroll_offset + 1).min(l.len().saturating_sub(1)); } }
                                                KeyCode::PageUp => { debug_auto_scroll = false; debug_scroll_offset = debug_scroll_offset.saturating_sub(20); }
                                                KeyCode::PageDown => { if let Ok(l) = logs.lock() { debug_scroll_offset = (debug_scroll_offset + 20).min(l.len().saturating_sub(1)); } }
                                                KeyCode::End => { debug_scroll_offset = usize::MAX; debug_auto_scroll = true; }
                                                _ => {}
                                            }
                                        }
                                    }
